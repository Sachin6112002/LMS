    ...
    <div className="p-2 sm:p-4 md:p-8 bg-green-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-8">
        <button
          ...
          className="bg-green-600 hover:bg-green-700 text-white px-4 py-3 rounded-lg font-semibold shadow flex items-center gap-2 justify-center w-full"
        >
          <FaUsers />
          Manage Users
        </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-cyan-50 p-4 sm:p-6 rounded-xl border border-cyan-100 flex flex-col items-center">
          <div className="text-2xl sm:text-3xl font-bold text-blue-700 mb-1">{stats.totalUsers}</div>
          <div className="text-gray-600">Total Users</div>
        </div>
        <div className="bg-cyan-50 p-4 sm:p-6 rounded-xl border border-cyan-100 flex flex-col items-center">
          <div className="text-2xl sm:text-3xl font-bold text-purple-700 mb-1">{stats.totalCourses}</div>
          <div className="text-gray-600">Total Courses</div>
        </div>
        <div className="bg-cyan-50 p-4 sm:p-6 rounded-xl border border-cyan-100 flex flex-col items-center">
          <div className="text-2xl sm:text-3xl font-bold text-green-700 mb-1">₹{stats.totalEarnings}</div>
        ...
        <table className="min-w-full bg-white rounded-xl text-xs sm:text-sm">
          <thead className="bg-cyan-50">
            <tr>
              ...
              <th className="px-2 sm:px-4 py-2 text-left">Course Title</th>
              <th className="px-2 sm:px-4 py-2 text-left">Enrollment Date</th>
          ...
          <tbody>
            {stats.enrolledStudentsData.map((item, idx) => (
              <tr key={idx} className="hover:bg-cyan-50 transition">
                <td className="px-2 sm:px-4 py-2 font-bold text-blue-700">{idx + 1}</td>
                <td className="px-2 sm:px-4 py-2">{item.student.name}</td>
                <td className="px-2 sm:px-4 py-2">{item.courseTitle}</td>
                <td className="px-2 sm:px-4 py-2">{item.enrollmentDate}</td>
                <td className="px-2 sm:px-4 py-2">
                  <span className={`px-2 sm:px-3 py-1 rounded-full text-xs font-bold ${item.status === 'completed' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>{item.status}</span>
                </td>
              </tr>
            ))}
          </tbody>

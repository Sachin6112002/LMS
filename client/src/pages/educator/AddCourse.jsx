 ...
 chapters.map((chapter) => ( chapter.chapterId === chapterId ? {...chapter , collapsed : !chapter.collapsed} : chapter))
  ...
  <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
    <form onSubmit={handleSubmit} className='flex flex-col gap-3'>
      ...
      <div className='flex flex-col gap-1'>
        <p>Course Price</p>
        <input onChange={e => setCoursePrice(e.target.value)} value={coursePrice}  type="number" className='outline-none  w-28 md:py-2.5 py-2 pl-2 rounded border border-gray-500'  required/>
      </div>
      <div className='flex  gap-2 items-center'>
        <p>Course Thumbnail</p>
        <label htmlFor="thumbnailImage" className='
      ...
      <div className='flex flex-col gap-1'>
        <p>Discount %</p>
        <input onChange={e => setDiscount(e.target.value)} value={discount}  type="number"  placeholder='0' min={0} max={100} className='outline-none  w-28 md:py-2.5 py-2 pl-2 rounded border border-gray-500'  required/>
      </div>
      {/* Adding the chapter  */}
      <div>
              ...
              <div className='p-4'>{
                chapter.chapterContent.map((lecture , lectureIndex)=>(
                  <div key={lectureIndex} className='flex justify-between items-center mb-2 '>
                    <span> 
                      {lectureIndex + 1} - {lecture.lectureDuration} min - <a href= {lecture.lectureUrl}
                ...
                <div className='fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50'>
                  <div className='bg-white text-gray-700 p-4 rounded relative w-full max-w-80'>
                   ...
                   <p>is Preview Free?</p>
                    ...
                    value={lectureDetails.lectureUrl} onChange={(e) => setLectureDetails({ ...lectureDetails ,isPreviewFree : e.target.checked})}/>
                   </div>

                  <button onClick={addLecture} type='button' className='w-full bg-blue-400 text-white px-4 py-2 rounde ' >Add</button>

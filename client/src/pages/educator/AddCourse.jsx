    ...
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0 bg-green-50'>
      {!createdCourse ? (
        <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-green-700'>
          ...
          <div>
            {chapters.map((chapter) => (
              <div key={chapter.chapterId || chapter.chapterOrder} className="bg-white border border-green-200 rounded-lg mb-4">
                ...
                {!chapter.collapsed && (
                  <div className="p-4">
                    ...
                    <div className="inline-flex bg-green-100 p-2 rounded cursor-pointer mt-2 text-green-700" onClick={() => handleLecture('add', chapter.chapterId)}>
                      + Add Lecture
                    </div>
                  </div>
                )}
